...
std::string JSONParser::inputString = "";
std::string::iterator JSONParser::inputIter = inputString.begin();
...
std::string JSONParser::internal_parseString() {
...
JSONParser::JSONObject JSONParser::internal_parseNull() {
	inputIter+=4;
	return JSONObject(Null);
}
JSONParser::JSONObject JSONParser::internal_parseBool() {
	if (*inputIter == 't') {
		inputIter+=4;
		return JSONObject(1);
	} else {
...
float JSONParser::internal_parseNumber() {
...
JSONParser::JSONObject JSONParser::internal_parseArray();

JSONParser::JSONObject JSONParser::internal_parseDictionary() {
...
JSONParser::JSONObject JSONParser::parseString(std::string input) {
...
JSONParser::JSONObject JSONParser::fromFile(std::string path) {
	std::string res="";
	std::string raw="", temp="";
...
std::string JSONParser::toString(JSONObject root) {
	std::string res="";
	root.toString(res);
	return res;
}
...
JSONObject::JSONObject(ValueType t): type(t) {}
JSONObject::JSONObject(std::string s) {
	type = String;
	valueString=s;
}
JSONObject::JSONObject(float f) {
	type = Number;
...
JSONObject& JSONObject::operator[](float ind) { // array or dict
...
JSONObject& JSONObject::operator[](std::string key) { // dict
...
JSONObject& JSONObject::operator[](JSONObject key) { // dict
...
void JSONObject::append(ValueType t) {
...
std::vector<JSONObject> JSONObject::keys() {
...
float JSONObject::getValueNumber() {
	return std::stof(valueString);
}
std::string JSONObject::getValueString() {
	return valueString;
}

void JSONObject::toString(std::string& curr) {
	if (type==Array) {
		curr+="[";
...
void JSONObject::print(int indent=0, bool doComma=false, bool skipFirst=false) {
	if (type==Array) {
